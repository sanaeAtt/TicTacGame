...
public class robotHard extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_robot_hard);
    ...
    private boolean playWin() {
        // Vérification pour win
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (buttons[i][j].getDrawable() == null) {
                    buttons[i][j].setImageResource(choixSecndPlayer);
                    if (checkWiner()) {
                        check();
                        return true;
                    } else {
    ...
    private boolean blockWin() {
        // Vérificaton win joueur
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (buttons[i][j].getDrawable() == null) {
                    buttons[i][j].setImageResource(choixFirstPlayer); // Jouer b7al joueur
                    if (checkWiner()) {
                        // Bloque
                        buttons[i][j].setImageResource(choixSecndPlayer);
                        check();
